#ifndef Komodo_API_EXCEPTION_H
#define Komodo_API_EXCEPTION_H
...
class KomodoException: public std::exception
{
private:
   int code;
   std::string msg;
...
public:
   explicit KomodoException(int errcode, const std::string& message) {
       this->code = parseCode(message);
       this->msg = parseMessage(message);
   }
   ~KomodoException() throw() { };
